...
export default function ProductAnalyticsPage() {
  ...
  const [error,setError] = useState(''); const [selected,setSelected] = useState(null); const [detail,setDetail] = useState(null); const [settlementOrder,setSettlementOrder] = useState(null);
  ...
  return <div className="product-analytics-page">
    ...
    {selected&&<div className="pa-modal-backdrop" onMouseDown={e=>e.target===e.currentTarget&&setSelected(null)}><section className="pa-detail"><header className="pa-detail-head"><div><span className="pa-eyebrow">SKU DETAIL</span><h2>{selected.product_name_kr||selected.item_name}</h2><p>{selected.option_name||'옵션 없음'}</p><div className="pa-detail-sku"><code>{selected.sku}</code><CopyIconButton value={selected.sku} label="SKU" /></div></div><button className="modal-close" aria-label="닫기" onClick={()=>setSelected(null)}>×</button></header>{!detail?<div className="pa-empty">상세 정보를 불러오는 중입니다…</div>:<><h3>국가별 판매 상품</h3>{detail.countries.length?<div className="pa-country-grid">{detail.countries.map((c,i)=><article key={`${c.region}-${c.shop_id}-${i}`}><b>{c.region}</b><span>Shopee 상품 ID <strong>{c.item_id||'-'}</strong></span><span>옵션 ID <strong>{c.option_id||'-'}</strong></span><span>판매 {num(c.sold_qty)}개 · 매출 {won(c.sales_krw)}</span><span>순이익 <strong className={profitTone(c.net_profit_krw)}>{won(c.net_profit_krw)}</strong></span></article>)}</div>:<div className="pa-empty compact">판매 국가 정보가 없습니다.</div>}<h3>최근 주문</h3><div className="pa-table-wrap small"><table><thead><tr><th>주문번호</th><th>국가</th><th>일시</th><th>수량</th><th>순이익</th></tr></thead><tbody>{detail.orders.slice(0,20).map((o,i)=><tr key={`${o.order_sn}-${i}`}><td><button className="pa-order-link" onClick={()=>setSettlementOrder({orderSn:o.order_sn,shopId:o.shop_id})}>{o.order_sn}</button></td><td>{o.region}</td><td>{String(o.order_created_at||'').slice(0,10)}</td><td>{num(o.qty)}</td><td>{o.net_profit_krw==null?'정산 대기':won(o.net_profit_krw)}</td></tr>)}</tbody></table></div>{detail.batches?.length>0&&<><h3>FIFO 입고 배치</h3><div className="pa-batches">{detail.batches.map(b=><span key={b.id}>{String(b.received_at||'').slice(0,10)} · 잔여 {num(b.remaining_qty)} · {won(b.unit_cost)}</span>)}</div></>}</>}</section></div>}
    {settlementOrder&&<OrderSettlementDetailModal orderSn={settlementOrder.orderSn} shopId={settlementOrder.shopId} onClose={()=>setSettlementOrder(null)} />}
  </div>;
}
